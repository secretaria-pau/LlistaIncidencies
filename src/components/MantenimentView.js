...
const MantenimentView = ({ onBackClick, profile, accessToken, users }) => {
  ...
  const handleSaveIncident = async (incidentData) => {
    ...
    try {
      let response;
      if (incidentData.ID) {
        response = await updateMantenimentIncident(incidentData, accessToken);
      } else {
        response = await addMantenimentIncident(incidentData, accessToken);
      }
                  ...
                  <React.Fragment key={estat}>
                    ...
                    {filteredIncidents.filter(inc => inc.Estat === estat).map(incident => (
                      <tr key={incident.ID} onClick={() => handleRowClick(incident)} style={{ cursor: 'pointer' }}>
                        ...
                        <td>{incident["Objecte avariat"]}</td>
                        <td>
                          <span 
                            className="info-icon"
          ...
          {selectedIncident && (
            <div className="card">
              ...
              <div className="card-body">
                ...
                <p><strong>Objecte avariat:</strong> {selectedIncident["Objecte avariat"]}</p>
            ...
            <div className="card">
              ...
              <div className="card-body">
                <form onSubmit={(e) => { e.preventDefault(); handleSaveIncident(editingIncident); }}>
                  ...
                  <div className="mb-3">
                    <label className="form-label">Objecte avariat</label>
                    <textarea className="form-control" value={editingIncident["Objecte avariat"] || ''} onChange={(e) => setEditingIncident({...editingIncident, "Objecte avariat": e.target.value})}></textarea>
                  </div>
                  <div className="mb-3">
